...
export const handlers = [
  ...
  http.get("/api/dashboard", () => { if (!session) return bad(409, "NO_ACTIVE_SESSION"); const battery = currentBattery(); const latest = battery.latest ?? { voltageV: null, currentA: null, powerW: null, representativeTempC: null, representativeTempSource: null, tempContact: null, tempIrSurface: null, socPct: null, socBasis: null, score: 0, grade: "NORMAL" as const, measuredAt: now() }; return HttpResponse.json({ session, battery, metrics: { voltageV: { value: latest.voltageV, status: null }, currentA: { value: latest.currentA, status: null }, powerW: { value: latest.powerW, status: null }, tempContact: { value: latest.tempContact ?? null, status: metricStatus(latest.tempContact ?? null) }, tempIrSurface: { value: latest.tempIrSurface ?? null, status: metricStatus(latest.tempIrSurface ?? null) }, representativeTempC: { value: latest.representativeTempC, source: latest.representativeTempSource, status: metricStatus(latest.representativeTempC) }, socPct: { value: latest.socPct, status: null }, socBasis: latest.socBasis, measuredAt: latest.measuredAt }, anomaly: { score: latest.score, grade: latest.grade, evaluatedAt: battery.latest ? latest.measuredAt : undefined }, relay, notices: [{ id: "n1", category: "MAINTENANCE", title: "7월 정기 서버 점검 (무중단)", summary: "WebSocket 순단이 발생할 수 있습니다.", publishedAt: "2026-07-01T00:00:00.000Z" }], snapshotCursor: "1" }); }),
  ...
  http.get("/api/alerts/summary", () => HttpResponse.json({ unacknowledgedCount: 1, today: { DANGER: 1, WARNING: 2, NORMAL_OR_CHECK: 2 } })),
