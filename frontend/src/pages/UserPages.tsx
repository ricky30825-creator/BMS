...
import type { Alert, AlertChannels, Battery, BatteryEvent, Dashboard, DashboardMetrics, Diagnosis, Grade, MeResponse, NoticeCategory, Relay, TrendResponse, VoiceAlertSettings } from "../types";
...
// 발열값은 부호가 정보다(음수 = 방이 배터리보다 따뜻함, 실온 드리프트). 0으로
// 자르지 않고 +/− 기호를 명시한다. 절댓값만 보이는 전류와 반대 규칙이다.
function signedTemperature(value: number | null | undefined): string {
  if (value == null) return "—";
  const text = Math.abs(value).toFixed(1);
  return text === "0.0" ? text : `${value > 0 ? "+" : "\u2212"}${text}`;
}
function HeatRiseStrip({ metrics }: { metrics: DashboardMetrics }) {
  const ambient = metrics.tempAmbientC.value;
  return <div className="dashboard-heat-rise" role="group" aria-label="실온과 발열값"><div><small>실온</small><strong className="mono">{ambient == null ? "실온 미측정" : <>{metricValue(ambient)}<em>°C</em></>}</strong></div><div><small>발열값 · 대표 온도 − 실온</small><strong className="mono">{signedTemperature(metrics.heatRiseC.value)}{metrics.heatRiseC.value != null && <em>°C</em>}</strong></div></div>;
}
...
export function dashboardTrendData(data: Dashboard, metric: DashboardMetricKey): Array<{ at: string; value: number | null }> {
  const requestedMetric = dashboardMetricParam(metric);
  if (data.quickTrend?.metric !== requestedMetric) return [];
  return rightAlignTrend(data.quickTrend.points, data.session.startedAt).map((point) => ({ at: point.at ? formatTime(point.at) : "", value: point.value }));
}
...
export function DashboardPage({ realtime, me }: { realtime: { state: RealtimeState; dashboard: Dashboard | null; lastAt: string | null; refetchMetric: (metric: DashboardMetricParam) => Promise<void> }; me: MeResponse }) {
  ...
  if (data.session.measurementPhase !== "MEASURING" || (!hasSensorMeasurement && !hasAnomalyMeasurement)) return <div className="page-stack"><PageHeading eyebrow="LIVE MONITORING" title="실시간 관제" description={`${data.battery.label} · battery_id ${data.battery.id} · 세션 진행 중`} actions={<span className={`stream-status stream-${realtime.state}`}><span className="status-dot" />{realtime.state === "live" ? "실시간 수신 중" : realtime.state === "reconnecting" ? "재연결 중" : "연결 대기"}</span>} /><Card className="gate-card"><EmptyState title="아직 측정 데이터가 없습니다." description="장비 연결을 기다리고 있습니다. 세션 시작 이후 첫 센서 스냅샷이 도착하면 측정값이 표시됩니다." /></Card></div>;
