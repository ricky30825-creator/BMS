...
import { AlertTriangle, ArrowDown, ArrowUp, BarChart3, BatteryCharging, Check, Download, FileText, LockKeyhole, Pause, Play, RefreshCw, ShieldAlert, ShieldCheck, Thermometer, TrendingUp, WifiOff, X } from "lucide-react";
...
export function DashboardPage({ realtime, me }: { realtime: { state: RealtimeState; dashboard: Dashboard | null; lastAt: string | null }; me: MeResponse }) {
  ...
  if (!hasSensorMeasurement && !hasAnomalyMeasurement) return <div className="page-stack"><PageHeading eyebrow="LIVE MONITORING" title="실시간 관제" description={`${data.battery.label} · 세션 진행 중`} actions={<span className={`stream-status stream-${realtime.state}`}><span className="status-dot" />{realtime.state === "live" ? "실시간 수신 중" : realtime.state === "reconnecting" ? "재연결 중" : "연결 대기"}</span>} /><Card className="gate-card"><EmptyState title="아직 측정 데이터가 없습니다." description="세션은 연결됐지만 서버에서 첫 센서 스냅샷을 기다리고 있습니다. 데이터가 도착하면 이상점수와 V · I · T · SOC 카드가 표시됩니다." /></Card><NoticePreview notices={data.notices} /></div>;
  if (!hasAnomalyMeasurement) return <div className="page-stack"><PageHeading eyebrow="LIVE MONITORING" title="실시간 관제" description={`${data.battery.label} · 세션 진행 중`} /><Card className="gate-card"><EmptyState title="이상점수를 사용할 수 없습니다." description="센서 측정은 도착했지만 서버의 이상점수와 등급이 아직 산출되지 않았습니다." action={<Button variant="secondary" onClick={() => void query.refetch()}>다시 시도</Button>} /></Card><NoticePreview notices={data.notices} /></div>;
...
function NoticePreview({ notices }: { notices: Dashboard["notices"] }) { const navigate = useNavigate(); return <Card><div className="card-title-row"><h2>공지사항</h2><button className="text-button" onClick={() => navigate("/notices")}>전체 보기 →</button></div>{notices.length ? <div className="notice-preview-list">{notices.map((notice) => <button key={notice.id} className="notice-preview" onClick={() => navigate("/notices")}><span className={`category-dot category-${notice.category.toLowerCase()}`} /><span><strong>{notice.title}</strong><small>{formatDateTime(notice.publishedAt)}</small></span></button>)}</div> : <EmptyState title="등록된 공지가 없습니다." />}</Card>; }
...
export function BatteryPage() {
  ...
  return <div className="page-stack battery-page"><PageHeading eyebrow="ASSET MANAGEMENT" title="배터리 관리" description="저장된 배터리 선택 · 새 배터리 등록" actions={<Button variant="primary" onClick={openCreate}>+ 새 배터리 등록</Button>} />
    {list.error ? <Card><TableState state="error" message="배터리 데이터를 불러오지 못했습니다." /></Card> : !list.isPending && !list.data?.items.length ? <Card><EmptyState title="등록된 배터리가 없습니다." description="첫 배터리를 등록하면 측정을 시작할 수 있습니다." action={<Button variant="primary" onClick={openCreate}>배터리 등록</Button>} /></Card> : <>
      ...
      <div className="battery-grid">{list.isPending ? <TableState state="loading" /> : batteries.map((battery) => <Card className={`battery-card ${battery.isConnected ? "connected" : ""}`} key={battery.id}><div className="battery-card-head"><div className="battery-icon"><BatteryCharging size={22} /></div><div><h2>{battery.label}</h2><p>{battery.maker ? `${battery.maker} · ` : ""}{battery.model || (battery.targetMode === 1 ? "리튬이온" : "보조배터리")} · {battery.targetMode === 1 ? "모드 1" : "모드 2"}</p></div>{battery.latest?.grade != null && battery.latest?.score != null ? <StatusBadge grade={battery.latest.grade} score={battery.latest.score} /> : <span className="plain-status">측정 없음</span>}</div><div className="battery-values"><span><small>{battery.targetMode === 2 ? "상대 SOC" : "SOC"}</small><strong className="mono">{metricValue(battery.latest?.socPct, 0)}{battery.latest?.socPct == null ? "" : " %"}</strong></span><span><small>최근 측정</small><strong>{battery.latest?.measuredAt ? relativeTime(battery.latest.measuredAt) : "측정 없음"}</strong></span><span><small>이상점수</small><strong className={`mono ${battery.latest?.grade?.toLowerCase() ?? ""}`}>{score100(battery.latest?.score)}</strong></span></div><div className="battery-card-foot"><span className="battery-mode-chip">{battery.targetMode === 2 ? "모드 2 · 보조배터리" : "모드 1 · 외부 셀"}</span><div><Button variant="ghost" onClick={() => { setSelected(battery); setModal("edit"); }}>수정</Button><Button variant="primary" onClick={() => setConnect(battery)} disabled={battery.opsStatus === "BLOCKED"}>{battery.opsStatus === "BLOCKED" ? "연결 잠김" : battery.isConnected ? "연결됨" : "연결하고 측정"}</Button></div></div></Card>)}</div>
