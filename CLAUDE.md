...
- **하드웨어 프로필을 먼저 본다.** 아래 CH1=충전·CH2=방전·CH4=포트 선택은 `MODE2_FULL` 전용이다. `COMBINED_EXISTING_PARTS_V1`은 CH1·CH2를 모드 2에서 항상 OFF로 두고 CH3=마스터·CH4=방전 경로로 쓰며, MQ-2·확정 안전 문턱·연속 감시가 없어 **0.5A·10초 시운전까지만** 허용한다. F21 진단 API는 `409 SAFETY_PROFILE_NOT_READY`; Raw는 `gas_raw`·`pressure_raw`·`temp_contact`·`soc_pct`가 `null`, `diag_phase`·`load_target_a`도 `null`이다. 정본은 통합 조립 가이드와 모드 2 스펙 §2-0이다.
