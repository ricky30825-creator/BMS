...
- **등급 판정은 서버가 한다.** 사용자별 임계치 설정(`/api/settings/thresholds`)이 존재하므로 클라이언트가 임계값을 하드코딩하면 어긋난다. 응답에 `grade`를 항상 동봉한다.
- **v3의 3등급 판정(`≥70 위험 / ≥40 주의`)과 게이지 범례(`정상 0–39/주의 40–69/위험 70+`)는 버그다.** 4등급(0.3/0.6/0.8)이 유일한 정본이니 v3를 보고 따라 구현하지 말 것.
...
미결정 항목은 `docs/backend_contract.md` §9(Q1–Q21)에 모아 두었다. 새로 결정되면 표에서 지우고 본문에 반영한다.
