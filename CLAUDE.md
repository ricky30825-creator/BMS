...
> **한 번에 배터리 1개만 측정한다.** MLX90614(0x5A)·BQ27441(0x55)은 I2C 주소가 고정이라 같은 버스에 2개를 물릴 수 없다. 여분으로 구매한 센서는 예비품이며, 다중 배터리 동시 측정은 범위 밖이다(멀티플렉서 미사용).
...
- **100ms로 실제 갱신되는 센서는 INA226뿐이다.** DS18B20 750ms(12비트) / BQ27441 ~1s / MLX90614는 공장 기본값이 **864.9ms**. `dT_dt`·`d2T_dt2` 특징이 계단 파형이 되므로 `age_ms` 신선도 필드 도입 여부가 미결이다(백엔드 스펙 §13).
...
- **MLX90614로 공간 피크는 못 낸다.** 서모파일 소자 1개짜리라 데이터시트대로 "FOV 안 모든 물체의 평균"만 나온다. `temp_ir_surface`의 피크는 **시간축 피크**(100ms 창의 최댓값)를 뜻한다. 공간 평균 문제는 **셀에서 2cm 이내에 부착**해 FOV(35°, 스팟 지름 = 0.63 × 거리)가 셀만 보게 해서 줄인다.
...
- **셀 단위 데이터는 전부 범위 밖이다.** 히트맵도, 이벤트·알림의 `cellIndex`도 만들지 않는다. 온도는 `temp_contact`·`temp_ir_surface` 두 값뿐이며, 이벤트명에서 `· 셀 N`을 뺀다.
