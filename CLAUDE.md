...
**`설계 산출물/셀가드 프로토타입_v3.html`은 정본이 아니다(2026-08-28).** 화면을 확인해야 하면 `frontend/`를 띄운다(`docs/local_run.md`). 이 번들은 그 이전 단계의 산출물이며, **현재 워킹트리에서 삭제된 상태다**(`git status`의 `D`). 문서에 남은 `[v3 실측]` 인용은 당시 근거 표시일 뿐 "지금 v3를 보라"는 뜻이 아니다.
...
- 브라우저 확인은 `설계 산출물/`에서 `python3 -m http.server 8807 --bind 127.0.0.1`(`file://`은 확장에 차단됨). 파일이 없으면 `git restore`로 복구된다 — *"`.gitignore` 대상이라 되돌리기 불가능"*이라던 옛 서술은 **틀렸다**(무시 규칙보다 먼저 추적됐다).
