...
- **DB는 `npm run db:migrate` 하나로 올린다(2026-08-28).** `migrations/000`~`009`를 파일명 순서대로 적용하고 `schema_migrations`에 기록한다. **`001_app_auth.sql`은 고치지 않는다** — 백엔드의 기준 파일이라 변경은 앞뒤 번호 파일로 쌓는다. `005`는 TimescaleDB 확장이 있어야 통과하며, 없으면 `005`만 실패하고 이후 migration까지 도달하지 못한다.
...
- **`DATA_MODE=postgres`는 `initializeStore()`가 migrations `000`~`009`의 핵심 스키마를 확인한 뒤에만 listen한다.** PostgreSQL 저장소(`backend/src/store/postgres.ts`)가 도메인 REST/WS 조회·변경을 담당하며, 연결·스키마 검사가 실패하면 memory 데이터로 대체하지 않고 기동을 중단한다. 정본은 `docs/handover/infra-implementations.md` §9.
