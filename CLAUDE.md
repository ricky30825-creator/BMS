...
문서로 정할 수 있는 항목은 2026-07-28에 전부 확정했다(백엔드 스펙 §13). 남은 건 **하드웨어를 만져야 답이 나오는 것뿐**이며 `docs/hardware/mode1_backend_spec.md` §14에 H1~H10으로 모아 두었다. 그중 조립을 막는 두 건:

- **셀에 보호회로(PCM)가 있는가**(H9). 회로에 퓨즈가 없어 이게 유일한 과전류 보호다. **18650 길이가 65mm면 없고 68~70mm면 있다.** 없으면 셀 교체나 폴리퓨즈 구매가 필요하고, 있으면 홀더 규격과 센서 배치 치수가 바뀐다.
- **Battery Babysitter 실물 실크스크린**(H10). 회로도 네트 이름(`BAT+`·`SYS+`·`GND_IN` 등)이 보드에 그대로 인쇄돼 있는지 확인 못 했다. 다르면 초보자가 배선표에서 연결 지점을 못 찾는다.
