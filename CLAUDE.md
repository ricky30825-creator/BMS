...
| 주제 | 정본 |
|---|---|
| **화면·동작 최종본** | **`frontend/`** — 실제 React 구현. v3 프로토타입 번들이 아니다(2026-08-28 확정) |
...
| DB 스키마 (실제 컬럼·제약) | `backend/migrations/` — `000`~`014`. `npm run db:migrate`로 적용한다(`backend/scripts/migrate.mjs`). `backend/src/store/types.ts`와 **한 쌍**이라 한쪽만 고치면 조용히 깨진다 |
...
**충돌 해소 순서**: 회로·실물 제약 > **`frontend/` 구현** > 계약서 > `PLAN.md` > 기능정의서·유저플로우·나머지.

> **화면·동작의 최종본은 `frontend/`다(2026-08-28 확정).** `frontend/`와 계약서가 어긋나면 **계약서를 현실에 맞춰 고친다** — 반대가 아니다. 회로·실물 제약만 그보다 위다.
>
> ⚠️ **`설계 산출물/셀가드 프로토타입_v3.html`은 더 이상 정본이 아니다.** 예전에는 이 번들이 화면 실측 근거였고 문서 곳곳에 `[v3 실측]` 인용이 남아 있지만, **지금 v3와 `frontend/`가 다르면 `frontend/`가 맞다.** 아래에 남은 v3 관련 경고들(목업 숫자, 3등급 판정 버그, `packBase` 하드코딩 등)은 *"v3를 보고 따라 구현하지 마라"*는 뜻이며 그대로 유효하다.

구현·문서 어느 쪽으로도 동작이 불명확하면 추정하지 말고 `정의 필요`로 표시한다.
...
outbox의 durable `event_id`는 version-1 JSON payload를 바꾸지 않고
Kafka `x-cellguard-event-id` header로 전달하며, producer 성공 전에는 `sent_at`을
기록하지 않는다.
...
- **하드웨어 프로필을 먼저 본다.** 아래 CH1=충전·CH2=방전·CH4=포트 선택은 `MODE2_FULL` 전용이다. `COMBINED_EXISTING_PARTS_V1`은 CH1·CH2를 모드 2에서 항상 OFF로 두고 CH3=마스터·CH4=방전 경로로 쓰며, MQ-2·확정 안전 문턱·연속 감시가 없어 **실물 부하는 0.5A·10초 시운전까지만** 허용한다. Raw는 `gas_raw`·`pressure_raw`·`temp_contact`·`soc_pct`가 `null`, `diag_phase`·`load_target_a`도 `null`이다. 정본은 통합 조립 가이드와 모드 2 스펙 §2-0이다. ⚠️ **이 프로필 제약은 실물 배선 이야기이며, 아래 소프트웨어 실행 잠금 해제와 별개다.**
...
- **빠른 진단 사다리는 더 이상 고정 전류가 아니다 — P7의 부하는 동적이다(2026-09-02).** 시퀀스가 `P0 P1 P2 P3 P4 **P7** P5`(총 180초)이고, **P7은 사다리가 확정한 붕괴점(`regulationKneeA`)의 0.9배**로 40초 도는 발열 탐침 구간이다(못 찾았으면 천장 2.0A). 붕괴점 대비로 걸어야 팩마다 자극이 정규화된다 — 같은 절대 전류는 정격 1A 팩엔 과부하, 3A 팩엔 경부하다. **동적인 건 `loadTargetA` 하나뿐이고 지속시간은 정적이다**(단계를 경과시간에서 계산하는 `phases.ts` 불변). `P6`(미세 스윕, 미구현)과 `P7`은 다른 단계다.
  - ⚠️ **P7을 `regulationKnee()`·`specAttainmentPct()`의 후보에 넣으면 안 된다.** ① P7 부하가 붕괴점의 0.9배인데 P7이 다시 붕괴점 후보가 되면 매 tick 0.9배씩 줄어드는 되먹임이 생기고 ② 도달률의 "지속 도달 전류" 최댓값이 올라가 **등급 산식을 한 줄도 안 고쳤는데 등급이 달라진다.** `metrics.ts`가 P7을 명시적으로 거른다. (P5는 원래부터 후보였다 — 빼면 그게 등급 변경이다.)
  - **P7·P5가 내는 `thermalProbeLoadA`·`thermalPerWattCPerMinPerW`·`recoverySlopeCPerMin` 3개는 원값이며 어떤 등급에도 연결돼 있지 않다.** 판정 방식이 미정이라 일부러 비워뒀다(스펙 §8 H21). **`thermalSlopeCPerMin`은 여전히 P3(1.5A 고정) 기준**이고 열화 등급에 쓰이는 건 이쪽뿐이다 — P7 값과 헷갈리면 조용히 틀린다.
  - ⚠️ **시뮬레이터의 `recoverySlopeCPerMin`은 항상 양수다.** 온도 모델이 `주변온도 + 계수 × 현재전력 × 총경과시간`이라 냉각도 열 기억도 없어, 부하를 내려도 온도가 오른다. 실물이면 "부하를 내렸는데 내부 발열이 계속된다"는 최강 적신호지만 여기서는 인공물이다. **중단 조건으로 승격하기 전에 `backend/src/diagnosis/simulator.ts`부터 고쳐라** — 안 그러면 모든 시뮬레이션 진단이 중단된다.
...
- **ATORCH BW150은 데이터 경로가 아니다.** 방전 부하 + INA226 검증용 기준기로만 쓴다. 시리얼(`0xFF 0x55` 프레임)로 값을 읽을 수는 있으나 **주기가 1초**라 100ms 스트림에 못 섞이고, **USB 절연이 없어** 측정 회로와 GND가 묶인다. 추출한 값은 Kafka가 아니라 오프라인 검증용 CSV로만 남긴다.
  ...
  - **✅ PC 연결은 CH340G 시리얼이 아니라 USB HID다(2026-08-06 실물 확인).** `VID 0x0483 / PID 0x5750`(STM32 내장 USB), 벤더 정의 usage page `0xFF02`, **입출력 각각 64바이트 리포트, 리포트 ID 없음**. **`/dev/cu.*` 시리얼 포트는 생기지 않으므로 `pyserial`이 아니라 `hidapi`를 쓴다.** macOS에서 쓸 때는 리포트 ID가 없어 앞에 `0x00`을 붙여야 하고, `get_input_report`·`get_feature_report`는 지원하지 않는다. **⚠️ macOS `입력 모니터링` 권한이 없으면 `enumerate`·`open`·`write`는 전부 성공하는데 `read`만 조용히 아무것도 안 준다 — 기기가 데이터를 안 보내는 것과 증상이 똑같아 오진하기 쉽다.** 권한 대상은 파이썬이 아니라 그걸 띄운 GUI 앱이고, 한 번 거부되면 macOS가 다시 묻지 않아 손으로 설정해야 한다. `detect.py`가 `IOHIDCheckAccess`로 이 상태를 먼저 알려준다. **ATORCH 공식 PC 소프트웨어는 Windows 전용**이라 맥에서는 자체 스크립트가 필수다(`edge/bw150/detect.py`).
  - **⚠️ 프로토콜은 방향에 따라 두 개다(2026-08-05 조사).** 기기→호스트는 **Atorch 네이티브 `FF 55 01 …`(36바이트, 1초 자동 push, 폴링 불필요)**, 호스트→기기 **부하 제어는 PX-100 레거시 `B1 B2 <cmd> <d1> <d2> B6`(6바이트, 체크섬 없음)**다. `FF 55 11 …` 명령 프레임은 리셋·버튼용이지 **부하 ON/OFF가 아니다.** 시리얼 9600 8N1. 체크섬은 `(sum(frame[2:-1]) & 0xFF) ^ 0x44`.
  - **⚠️ 전력(W)과 저항(Ω)은 프레임에 없다 — `V×I`, `V/I`로 직접 계산해야 한다.** 화면에는 뜨지만 바이트로는 안 온다. 그리고 **오프셋 10-12는 누적 용량(Ah)이지 "Watt"가 아니다** — 일부 공개 문서가 전력이라 적어 두었으나 실측 샘플과 맞지 않는 오류다. 상세 오프셋 표는 추출 가이드 §3-1.
  - **Tuya 로컬 폴링은 1초 데이터 수집에 쓰지 마라.** tinytuya는 1초 미만 폴링을 권고하지 않고 전력 계측류는 5~10초가 권장이며, IoT Platform 트라이얼이 1개월 단위라 만료되면 수집이 멈춘다. **수집은 유선 시리얼, Tuya는 제어·원격 감시용.**
...
**모드 2 부하 수단과 릴레이 매핑은 2026-07-28에 확정됐다.** 보유 BW150을 사용하며 상세 회로·실물 게이트는 `docs/hardware/mode2_powerbank_diagnosis_spec.md`와 통합 조립 가이드를 따른다. 안전 문턱과 연속 감시가 검증되기 전 `COMBINED_EXISTING_PARTS_V1`을 제품 F21 진단으로 승격하지 않았으나, 소프트웨어 실행 잠금은 2026-09-01에 열렸다(위 절 참조). 실물 부하를 거는 것과 시뮬레이션 실행은 다른 문제다.
...
## 프로토타입 번들 — 폐기됨, 열지 않아도 된다

**`설계 산출물/셀가드 프로토타입_v3.html`은 정본이 아니다(2026-08-28).** 화면을 확인해야 하면 `frontend/`를 띄운다(`docs/local_run.md`). 이 번들은 그 이전 단계의 산출물이며, **2026-09-02에 저장소에서 삭제됐다**(22MB). 히스토리에는 남아 있으므로 필요하면 아래 명령으로 되살릴 수 있다. 문서에 남은 `[v3 실측]` 인용은 당시 근거 표시일 뿐 "지금 v3를 보라"는 뜻이 아니다.

굳이 열어야 할 때만 아래를 지킨다 — 23MB 단일 HTML이라 평범한 편집이 통하지 않는다.

- 실제 마크업은 **210번 줄의 JSON 문자열 하나**(약 30만 자 / 2,372줄)다. 반드시 `tools/bundle_io.py`의 `unpack`/`pack`/`backup`으로만 읽고 쓴다.
- **202번 줄은 22MB base64 자산 매니페스트다. 절대 건드리지 않는다.**
- 마크업은 `{{ 바인딩 }}` 자리표시자를 쓰므로 한국어로 grep하면 안 나온다. **바인딩 이름으로 찾는다.**
- **파일이 워킹트리에 없다 — 먼저 히스토리에서 꺼내야 한다.** 삭제 직전 커밋에서 되살린다:

  ```bash
  P="설계 산출물/셀가드 프로토타입_v3.html"
  git checkout "$(git rev-list -n 1 HEAD -- "$P")^" -- "$P"
  ```

  ⚠️ **`git restore`만으로는 안 된다** — 삭제가 커밋됐으므로 HEAD에도 파일이 없다(2026-09-02 이전에는 워킹트리 삭제가 커밋 안 된 상태라 `git restore`로 됐고, 그 옛 안내는 이제 틀리다). 확인이 끝나면 다시 지운다 — 22MB를 커밋에 되돌려 넣지 않는다.
- 브라우저 확인은 `설계 산출물/`에서 `python3 -m http.server 8807 --bind 127.0.0.1`(`file://`은 확장에 차단됨). *"`.gitignore` 대상이라 되돌리기 불가능"*이라던 더 옛 서술도 **틀렸다**(무시 규칙보다 먼저 추적됐다).
...
- **DB는 `npm run db:migrate` 하나로 올린다(2026-08-28).** `migrations/000`~`009`를 연속된 파일명 순서대로 적용하고 `schema_migrations`에 기록한다. **`001_app_auth.sql`은 고치지 않는다** — 백엔드의 기준 파일이라 변경은 앞뒤 번호 파일로 쌓는다. `005`는 TimescaleDB 확장이 필수이며, extension/hypertable 확인이 실패하면 `TIMESCALEDB_REQUIRED`로 중단한다. plain PostgreSQL 테이블이나 memory 데이터로 강등하지 않는다. 전체 로컬 통합 시작·중지·복구는 `docker-compose.local.yml`과 `docs/local_run.md`를 따른다.
...
- **`DATA_MODE=postgres`는 `initializeStore()`가 migrations `000`~`014`의 핵심 스키마(`013`·`014`의 `telemetry_metric`·`battery_latest` `temp_ambient` 포함)와 TimescaleDB extension/hypertable을 확인한 뒤에만 listen한다.** PostgreSQL 저장소(`backend/src/store/postgres.ts`)가 도메인 REST/WS 조회·변경을 담당하며, 연결·스키마 검사가 실패하면 memory 데이터로 대체하지 않고 기동을 중단한다. 정본은 `docs/handover/infra-implementations.md` §9.
