...
수정 항목 (계획 Phase 1):
 ...
 9. AI 이상탐지(n23) -> 임계치 설정(n43) 링크 추가
10. 모드 인터락(n17) -> 실시간 대시보드(n9) 링크 추가
11. 릴레이 차단(n36) "복구 요청" 라벨 -> "차단 요청"
12. 복구 확인(n39) "센서 수집 상태로"(n21) -> "관리자 콘솔로"(n35)
...
clone_link_after("n23", "임계치 설정", "n43", "9")    # 이상탐지 → 임계치
clone_link_after("n17", "실시간 대시보드로", "n9", "10")  # 모드인터락 → 대시보드

# 11) 릴레이 차단(n36) "복구 요청" 라벨 -> "차단 요청"
m = re.search(r'(<[^>]*class="wf-clickable" data-wf-action="n37"[^>]*>)복구 요청(</[a-z]+>)', html)
assert m, "릴레이차단 '복구 요청' 라벨 실패"
html = replace_once(html, m.group(0), m.group(1) + "차단 요청" + m.group(2), "11 라벨정정")
print("[11] 릴레이 차단 '복구 요청' → '차단 요청'")
