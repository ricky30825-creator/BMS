...
|---|---|---|---|---|
| 모드 1 | 외부 셀 (리튬이온 18650 / 리튬폴리머) | 접촉식(temp_contact) + IR 표면(temp_ir_surface) | ○ | ○ |
| 모드 2 | 외부 보조배터리 | IR 표면(temp_ir_surface) | ○ | ✕ |

> AI 서버 전처리: 칼만 필터(temp_ir_filtered), 내부 셀 추정(temp_cell_estimated)
> 가스 센서는 전 모드, 압력·음향 센서는 모드 1에만 적용(보조배터리는 물리 부착이 어려워 제외).
...
| 가스 센서 (MQ-2) [SZH-SSBH-026] | 3 | 오프가스(가연성가스·연기·H₂ 등) 누출 감지 → 열폭주 조기경보 | `gas_raw` | ADS1115 아날로그 | **1·2** |
| 압력 센서 (FSR 406) [30-73258] | 3 | 스웰링(부풀음) 압력/스트레인 변형률 | `pressure_raw` | ADS1115 아날로그 | **1** |
| 음향 센서 (미확보) | — | 표면 진동·파열음 | `acoustic_raw` | ADS1115 아날로그 / GPIO | **1** |
...
- **가스(전 모드)**: 가연성가스·연기·H₂ 등 오프가스 급상승을 감지하면 즉시 차단. 모드 1·2 전부 적용.
- **압력·음향(모드 1)**: 셀의 물리적 부풀음·표면 파열음을 직접 측정해 임계 초과 시 즉시 차단. 보조배터리(모드 2)는 외장 케이스에 가려 부착이 어려워 제외.
- **아날로그→I2C**: MQ 계열·FSR 406은 아날로그 출력이라 ADS1115(16비트 ADC)를 거쳐 수집한다.
...
**보조·실험 장비(BOM)**: 4채널 5V 릴레이 모듈(SZH-RLBG-012) ×1, 전자부하 테스터(ATORCH BW150 WiFi Standard) ×1, USB-C 충방전모듈(SZH-MIN002) ×2, PD USB-C 트리거(ZY12PDN) ×3, 0.96" OLED SPI(CN0219) ×1, 실리콘 절연 배선 0.75SQ 적/흑 각 1M, 점퍼 F/F 40P ×5.

**미확보 — 조립 전 확보 필요**: 음향 센서, 스피커(라즈베리파이 5는 3.5mm 잭이 없어 USB 또는 I2S DAC), FSR 분압용 고정저항(10kΩ 등), 18650 홀더 ×3(셀 직접 납땜은 열손상 위험), JST 2.0 커넥터(리튬폴리머 ↔ Battery Babysitter), 캡톤 테이프·서멀 패드(접촉 온도 센서 고정·열전도), 외장 케이스, C타입 어댑터(5V 3A+).
...
- **OLED(CN0219)는 SPI 7핀**(`GND VCC D0 D1 RES DC CS`)이라 I2C가 아니다.
...
**미해결 (`정의 필요`)**
- 모드 2(보조배터리)의 `soc_pct` 출처. Battery Babysitter는 셀 직결 보드라 완제품 보조배터리에 부착할 수 없다.
- 가스·압력·음향의 동시 부착 개수와 ADS1115 채널 배분. 현재 4채널 1개뿐이라 각 1개(3채널)를 넘으면 ADS1115 추가가 필요하다.
- **구매한 셀에 보호회로(PCM)가 붙어 있는가.** 18650 [ZM18650-2600-KC01], 리튬폴리머 [TW102050] 둘 다 실물 확인 필요. 모드 1 회로에 퓨즈가 없어 이게 유일한 과전류 보호이며, 없으면 보호회로 달린 셀로 교체하거나 폴리퓨즈(3A PPTC)를 새로 사야 한다. 있으면 18650이 68~70mm로 길어져 홀더·센서 배치 치수가 바뀐다.
- 음향 센서 모델. **미세 크랙의 음향 방출(AE)은 100kHz~1MHz 대역**이라 일반 사운드 센서(20Hz~20kHz)로는 잡히지 않는다. 피에조로 벤트 파열음·표면 진동을 잡는 수준이 현실적이라 `acoustic_raw`의 정의를 "표면 진동·파열음"으로 낮췄다.
...
> 가스(`gas_raw`)·압력(`pressure_raw`)·음향(`acoustic_raw`)은 AI 예측 입력 특징이 아니라 **사후 대응 안전계층**이다. 각 센서가 임계값을 초과하면 AI 판정과 무관하게 즉시 릴레이를 차단한다(적용 모드: 가스 1·2, 압력·음향 1). AI 이중 모델 입력은 전압·전류·온도·SOC 기반 특징만 사용한다.
...
| 리스크 | 대응 |
|---|---|
| 보조배터리 내부 BMS 정보 접근 불가 | 모드 2 외부 IR 온도 기반 추정으로 대체. `soc_pct` 출처는 `정의 필요` |
...
### 2026-07-28 — 모드 1 회로에서 직렬 퓨즈 제거, 과전류 보호를 셀 보호회로에 위임
...
⚠️ **이 결정은 셀에 보호회로가 실제로 있다는 전제에 의존한다.** 구매한 `18650 [ZM18650-2600-KC01]`과 `리튬폴리머 [TW102050]`에 보호회로가 붙어 있는지는 **실물로 확인해야 한다(`정의 필요`)**. 보호회로가 없는 맨 셀이면 배선 단락을 막을 소자가 회로 안에 하나도 없으므로, 보호회로 달린 셀로 바꾸거나 폴리퓨즈(3A 홀딩, PPTC)를 새로 사야 한다.

⚠️ **보호회로가 있으면 18650 길이가 65mm가 아니라 68~70mm다.** 18650 홀더 규격과 가이드 §4-5의 DS18B20 3점 배치 치수가 달라진다.
...
| **신규 부품** | 0.96" OLED(CN0219, **SPI**), 18650 3.7V 2550mAh ×3, 리튬폴리머 3.7V 1000mAh ×2 |

**미해결(`정의 필요`)**: 모드 2의 `soc_pct` 출처, 가스·압력·음향의 동시 부착 개수와 ADS1115 채널 배분, 음향 센서 모델.
