...
> **동시 측정은 배터리 1개다**. MLX90614(0x5A)·BQ27441(0x55)의 I2C 주소가 고정이라 같은 버스에 2개를 물릴 수 없다. 여분 수량은 예비품이며 멀티플렉서(TCA9548A)는 도입하지 않는다.
...
| MLX90614 [SEN0206] | 2 | 비접촉 IR 표면 온도 (구 'IR 카메라' 대체) | `temp_ir_surface` | I2C 0x5A | 1·2 |
...
- **동시 측정은 배터리 1개**이므로 여분 수량은 예비품이다. 표의 I2C 주소가 고정 주소이며, 같은 주소 부품을 2개 이상 동시에 물리지 않는다.
...
| **동시 측정 1개 확정** | MLX90614(0x5A)·BQ27441(0x55) 주소 고정으로 다중 연결 불가. 여분 수량은 예비품이며 멀티플렉서 미도입 |
