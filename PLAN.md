...
`mode2_powerbank_diagnosis_spec.md` §8에 H1~H10으로 모아 두었다. **H1의 V1이 유일하게 회로도를 막는다** — 제품 보드 실크스크린의 `input 8V<V<36V`가 부하 입력 범위인지 보드 보조전원 범위인지다. 보조배터리는 5V인데 판매 페이지는 "1-200V", 기기 화면에는 18650 전압 `04.11V`가 떠 있어 **셋이 서로 맞지 않는다.** 부하 범위로 확정되면 BW150이 탈락하고 MOSFET 정전류 + MCP4725로 간다. **동봉 설명서와 보드 뒷면만 보면 되는 확인이라 계측기가 필요 없다.** (V2 BW150의 DL24 명령셋 호환·V3 Tuya 제어는 실패해도 **사람이 돌리고 파이가 INA226 전류에서 계단을 검출**하는 방식으로 우회된다.)

나머지: H2 표면온도 중단 문턱(케이스가 열을 막아 셀 70°C에 표면 45°C가 가능 — 모드 1의 55/60°C를 쓰면 안 된다), H3 발열 기울기 문턱 `S1`, H4 부스트 효율 η 기본값(현재 가정 0.88), H6 MQ-2 가스 임계, H7 컷오프 회복 판정 시간, H8 광고 정격 전류 입력 방식, H9 최소 유지 부하 크기와 자동 차단 시간, H10 보조배터리 입출력 포트 구성. **H5(릴레이 매핑)는 위에서 닫혔다.**
