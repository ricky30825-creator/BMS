...
**미해결 (`정의 필요`) — 실물을 봐야 닫힌다**
- **구매한 셀에 보호회로(PCM)가 붙어 있는가.** 18650 [ZM18650-2600-KC01], 리튬폴리머 [TW102050] 둘 다 실물 확인 필요. **18650 길이가 65mm면 없고 68~70mm면 있다.** 모드 1 회로에 퓨즈가 없어 이게 유일한 과전류 보호이며, 없으면 보호회로 달린 셀로 교체하거나 폴리퓨즈(3A PPTC)를 새로 사야 한다. 있으면 홀더·센서 배치 치수가 바뀐다.
- **Battery Babysitter 실물 실크스크린.** 회로도 네트 이름(`BAT+`·`SYS+`·`GND_IN` 등)이 보드에 그대로 인쇄돼 있는지 미확인. 공식 Hookup Guide는 핀 라벨을 이미지로만 제공한다.
- 그 밖의 실측 항목 8건은 `docs/hardware/mode1_backend_spec.md` §14(H1~H10)에 모아 두었다 — 션트 실측값, `therm_bulk_read` 지원, Pi 5 `gpio=` 동작, MLX90614 정착시간, BQ27441 센스 저항, DS18B20 ROM↔위치, BW150 프레임, 압력 baseline·임계.
...
| `capacity_mah` / `capacity_wh` | **`target_mode`=2면 필수**, 모드 1이면 선택 — 모드 2의 `soc_pct`가 이 값을 분모로 쓴다(2026-07-28) |
...
**남은 것은 실물 확인 10건**(`docs/hardware/mode1_backend_spec.md` §14 H1~H10). 그중 **H9(셀 보호회로 유무)와 H10(Babysitter 실크스크린)은 사진 한 장이면 닫힌다.**
