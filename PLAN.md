...
- 백업본은 항상 아카이브로 이동한다. 특정 산출물 폴더에 `아카이브/`가 있으면 해당 폴더를 우선 사용하고, 없으면 루트 `archive/`를 사용한다.
...
- 로컬 수정본 `설계 산출물/중간 보고서-내파트_파스텔_UC세로.pptx`를 생성해 전체 주요 원색 계열 RGB를 파스텔 톤으로 낮췄다.
- UC-01~UC-16 유즈케이스 다이어그램을 세로형 PlantUML 소스로 재작성하고, 슬라이드 9~24의 오른쪽 다이어그램 이미지를 각 UC별 세부 흐름 이미지로 교체했다.
- PlantUML 소스와 렌더링 PNG는 공유 가능한 다이어그램 자산으로 `assets/diagrams/usecase/`에 보관한다.
- 백업본은 항상 해당 산출물 폴더의 `아카이브/` 또는 루트 `archive/`로 이동한다는 작업 규칙을 `AGENTS.md`와 `PLAN.md`에 반영했다.
- `설계 산출물/중간 보고서-내파트_파스텔_UC세로.pptx`의 기존본을 `설계 산출물/아카이브/`에 백업한 뒤, UC-01~UC-16 PlantUML 다이어그램의 폰트 크기와 PNG 캔버스(760×1780)를 통일했다.
- 동일 PPT의 유즈케이스 정의서 표(슬라이드 9~24)는 파스텔 헤더, 좌측 항목 열, 교차 행 배경, 흰색 그리드, 통일 글꼴/여백으로 재정리했다.
- `설계 산출물/중간 보고서-내파트_파스텔_UC세로.pptx`를 다시 `설계 산출물/아카이브/`에 백업한 뒤, UC-01~UC-16 PlantUML 다이어그램을 투명 배경 PNG로 재생성했다.
- 다이어그램 글자 크기 차이를 줄이기 위해 모든 UC 다이어그램의 단계 수를 7개로 맞추고, PlantUML 원본 픽셀 크기를 확대/축소하지 않은 채 동일 투명 캔버스(820×1760)에 배치했다.
- 유즈케이스 정의서 표는 사용자가 선택한 C안(핵심 강조형 표)으로 적용해 `목적`, `기본 흐름`, `대안/예외 흐름`, `후행조건` 행만 강조색과 굵은 글씨로 구분했다.
