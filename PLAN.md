...
- **압력 임계는 baseline 대비 상대 상승률이다.** FSR은 예압에 따라 baseline이 매번 달라져 절대 카운트가 무의미하다. baseline은 **세션마다** 시작 10초 중앙값으로 새로 잡는다. 구체 상승률 임계는 실측 후 확정(`mode1_backend_spec.md` §14 H8).
...
**미확보 — 조립 전 확보 필요**: 음향 센서, 스피커(라즈베리파이 5는 3.5mm 잭이 없어 USB 또는 I2S DAC), FSR 분압용 고정저항(10kΩ 등), **18650 홀더 ×3 — 반드시 보호회로 셀용(전체 68~70mm 수용). 65mm용은 안 들어간다**(셀 직접 납땜은 열손상 위험), JST 2.0 커넥터(리튬폴리머 ↔ Battery Babysitter), 캡톤 테이프·서멀 패드(접촉 온도 센서 고정·열전도), **FSR용 실리콘 고무 패드 2~3mm(곡면 선접촉 방지)**, 외장 케이스, C타입 어댑터(5V 3A+).
...
- **Battery Babysitter 실물 실크스크린.** 회로도 네트 이름(`BAT+`·`SYS+`·`GND_IN` 등)이 보드에 그대로 인쇄돼 있음을 확인. 가이드 §4-3의 `확인 필요` 제거.

**미해결 — 전원을 넣어 봐야 닫힌다**
- 실측 항목 8건이 `docs/hardware/mode1_backend_spec.md` §14(H1~H8)에 있다 — 션트 실측값, `therm_bulk_read` 지원, Pi 5 `gpio=` 동작, MLX90614 정착시간, BQ27441 센스 저항, DS18B20 ROM↔위치, BW150 프레임, 압력 baseline·임계. **조립 단계에서 하나씩 지운다.**
...
| `rated_output_current_a` | 선택 — 보조배터리 겉면의 광고 정격 출력 전류. 모드 2 빠른 진단의 **스펙 도달률** 산출에 쓴다. 없으면 그 지표는 `null`(2026-07-28) |
...
| 빠른 진단 지표 | 부하 계단 스윙 0.5→1.0→1.5→2.0A로 **① 레귤레이션 이탈 전류**(출력이 무부하값의 94% 아래로 무너지는 최소 전류) **② 발열 기울기**(1.5A 40초 구간 최소자승) **③ 스펙 도달률** |
...
**미해결 — 부품을 정하거나 전원을 넣어야 닫힌다**

`mode2_powerbank_diagnosis_spec.md` §8에 H1~H8로 모아 두었다. 그중 **H1(부하 수단)이 조립을 막는다** — 계단 스윙은 프로그램이 전류를 지정할 수 있는 부하를 요구하는데 현재 BOM에 그런 소자가 없다. BW150은 수동 설정이라 못 쓰고(이미 데이터 경로가 아니라고 확정됨), 고정저항 뱅크는 미세 스윕이 불가능하고 릴레이 채널이 부족하다. **권장은 MOSFET 정전류 부하 + MCP4725 I2C DAC**이며 부품 추가 구매가 필요하다. **H1이 닫히기 전에는 `hardware/mode2/` 회로도를 그릴 수 없다.**

나머지: H2 표면온도 중단 문턱(케이스가 열을 막아 셀 70°C에 표면 45°C가 가능 — 모드 1의 55/60°C를 쓰면 안 된다), H3 발열 기울기 문턱 `S1`, H4 부스트 효율 η 기본값(현재 가정 0.88), H5 모드 2 릴레이 매핑, H6 MQ-2 가스 임계, H7 컷오프 회복 판정 시간, H8 광고 정격 전류 입력 방식.
...
| **`voltage_v` 보정** | **20mΩ 보정식 적용해 발행.** 센스 저항은 설정 파일(기본 0.020Ω) |
...
**같은 날 실물 확인으로 H9·H10도 닫혔다** — 셀에 보호회로가 있고, Babysitter 실크스크린은 회로도 네트 이름과 일치한다. **남은 것은 전원을 넣어야 답이 나오는 8건**(`mode1_backend_spec.md` §14 H1~H8).
