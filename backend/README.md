...
fallback. Apply migrations through `012_failsafe_profile_and_baseline.sql`
...
Migrations are applied by `npm run db:migrate` in filename order (`000`..`012`), not by running individual `.sql` files by hand. `010_domain_events.sql` stores durable event transitions, `011_notices.sql` stores notices, view dedupe, and delivery intents, and `012_failsafe_profile_and_baseline.sql` extends the Fail-Safe profile constraint and stores session pressure baselines. The earlier instruction to run `001_app_auth.sql` after creating the Better Auth core tables is obsolete: `000_identity.sql` now creates `"user"` itself, and `001` depends on it.
