...
| PostgreSQL 저장소 계약 | `TEST_DATABASE_URL=... npm --prefix backend test` (migrations `000`~`014` 적용 DB) | memory 계약 스위트·전역 active-session 경합·Fail-Safe relay/audit/domain/outbox transaction·rollback/replay·공지/조회수 동시성 계약 통과. URL이 없으면 실제 연결 없이 명시적 skip |
...
| Migration fail-closed | `npm --prefix backend run db:migrate` against TimescaleDB | 연속된 migration `000`~`014`, advisory lock, per-file transaction, extension availability와 두 hypertable post-check 통과 | plain PostgreSQL/extension/hypertable 누락은 `TIMESCALEDB_REQUIRED`, memory fallback 금지 |
