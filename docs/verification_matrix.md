...
| Fail-Safe 설정 경계 | `npm --prefix backend test -- src/failsafe.test.ts src/failsafeRunner.test.ts src/telemetryConsumer.test.ts src/store/postgres.unit.test.ts src/outboxWorker.test.ts src/config/env.test.ts` | synthetic raw→fake PostgreSQL load→Fail-Safe→relay interlock→audit/domain event/outbox→fake Kafka publish/WS 경로; mode 1/2 센서 가용성, 각 0 sentinel, 10초 세션 baseline·500 미만 부착 event, duplicate/replay/out-of-order, already cut, write rollback, AI 부재 확인. 실제 broker·실물 relay 인수와 threshold 승인은 별도 |
