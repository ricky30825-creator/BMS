...
> **2026-09-16 현재 상태:** migrations `000`~`014`가 연속 적용 대상이다(`013`·`014` 실온 컬럼 2026-09-25 추가).
...
| `014_latest_ambient.sql` | 대시보드 발열값용 `battery_latest.temp_ambient`. 새 프레임에 실온이 없으면 null로 덮어쓴다(오래된 실온과 새 표면온도를 짝짓지 않기 위해) |
...
> `npm run db:migrate`가 `014`까지 완료되고 두 hypertable 확인을 통과해야 한다.
...
- ✅ **스키마** — `migrations/000`~`014`, `npm run db:migrate`로 적용. 결정 근거는 `schema-open-questions.md`와 §13·§16.
