...
## 추가 결정 — 2026-07-22 사용자 v3 검토 반영

사용자가 v3를 직접 검토해 결함 8건을 보고했다. 전부 마크업에서 재확인했다. 아래는 남은 Task에 반영할 사항이다. **해당 Task의 요구사항에 포함된다.**

### 확인된 v3 결함

| # | 결함 | 근거 (언팩 마크업) |
|---|---|---|
| 1 | 등급 판정이 3곳이고 서로 다름 | L1647–1650 계산(4등급, 정본) / L1653–1657 `packBase` 하드코딩 status / L1924·1927 보조 로직(3구간) |
| 2 | 게이지 범례가 3칸 | L1590 등 |
| 3 | 원본 데이터에 `anomaly_score` 82 (정수) | RAW 모달 L1281–1307 |
| 4 | 프로필 사진 변경이 남아 있음 | 설정 L947–988 (REQ-WEB-069는 삭제 결정됨) |
| 5 | 셀 온도 히트맵 | 배터리 상세 L725·736 (`셀 1`~`셀 4`) |
| 6 | 운영 상태 라벨 두 벌 공존 | NORMAL 12 / WATCH 11 / BLOCKED 16 대 주시 9 |
| 7 | 상대 시각이 하드코딩 | "2일 전" 등 |
| 8 | 이벤트 페이지네이션 불일치 | L1590 "1–5"인데 6행 렌더 |

PACK-003이 `score: 33, status: '정상'`인 것이 1번의 대표 증상이다. 4등급으로는 주의(30–59)다. 등급이 계산이 아니라 데이터에 박혀 있어 판정 로직을 고쳐도 바뀌지 않는다.

### 사용자 결정

1. **문서엔 있으나 v3에 없는 기능은 계약서가 요구한다.** 관리자 운영 상태 변경(REQ-WEB-123)·관리자 메모(124)·디바이스 음성 안내(072) 모두 `**반드시 가능한 동작**`에 넣는다. v3에 미구현이라는 사실은 부록 B에 별도로 기록하되, 제외 목록이 아니라 "계약서가 요구하나 v3에 아직 없음" 항목으로 구분해 적는다.
2. **v3 자체는 데이터 모순만 고친다.** 이 플랜의 범위 밖이며 별도 작업으로 처리한다. 계약서 작성은 v3 수정을 기다리지 않는다.

### 원본 값과 표시 값의 구분 — 블록 A 보정 필요

지금 블록 A는 "Final Score는 0.0–1.0, 표시는 0–100 정수"까지만 말한다. 원본 데이터 조회는 **표시가 아니라 원본**이므로 0.82여야 한다. 이 구분을 블록 A의 등급 절에 한 문장 추가한다.

> 원본 데이터를 그대로 보여주는 곳에서는 환산하지 않은 0.0–1.0 값을 쓴다. 0–100 정수는 사람이 읽는 표시에만 쓴다.

### Task별 반영 지점

| Task | 반영 |
|---|---|
| 3 (완료) | 블록 A에 위 원본/표시 구분 문장 추가 |
| 6 | F7 `**제약**`에 셀 단위 온도를 제공하지 않는다(접촉·IR 표면 온도만 측정). F4 `**제약**`에 상대 시각은 기준 시각에서 계산한다 |
| 7 | F10 `**제약**`에 표시 중인 범위와 실제 항목 수가 일치해야 한다. F10에 원본 데이터는 0.0–1.0. F14는 음성 안내를 요구하고 사진 변경은 넣지 않는다 |
| 8 | F17 `**제약**`에 운영 상태 라벨은 한 벌만 쓴다. F17에 운영 상태 변경과 관리자 메모를 `**반드시 가능한 동작**`으로 넣는다 |
| 9 | 부록 B에 사진 변경·셀 온도 히트맵을 제외 항목으로, 관리자 입력 2종·음성 안내를 "요구하나 v3 미구현"으로 구분해 기록 |
...
> **REQ 배정 정정 (2026-07-22, `docs/backend_contract.md` §11 반영)**
> - 제외 5건은 어느 영역에도 배정하지 않는다: 030·031(devices 고아), 069(사진 변경), 071(캘리브레이션), 135(감사 상세). 린터의 `EXCLUDED_REQS`가 본문 인용을 막는다. 부록 B에서만 언급한다.
> - 실제 화면 위치가 문서와 다른 4건을 옮겼다: 026은 이상 탐지(F8), 051은 배터리 자산관리(F6), 054·055는 추세(F9), 047은 실시간 관제(F4).
> - 017(메뉴 접기)은 이동 수단 구조가 자유이므로 부록 B에 제외 사유와 함께만 적는다.
> - 기대 총계는 104건이다.
...
인용할 REQ:
- F6 배터리 자산관리 — 032, 033, 034, 035, 036, 037, 038, 039, 051, 073
- F7 배터리 상세·이력 — 040, 041
- F8 이상 탐지 — 026, 042, 043, 044, 045
...
인용할 REQ:
- F9 추세 — 046, 048, 054, 055
- F10 이벤트 이력 — 049, 050, 052, 053
- F11 알림 센터 — 016, 056, 057, 058
- F12 릴레이 제어 — 061, 062, 063, 064
- F13 공지사항 — 059, 060
- F14 설정 — 015, 065, 066, 067, 068, 070, 072
